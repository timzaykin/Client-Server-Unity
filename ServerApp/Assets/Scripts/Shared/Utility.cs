...
public static class Utility
{
    ...
    public static bool IsEmail(string email) {
        if (email != null) return Regex.IsMatch(email, EMAIL_PATTERN);
        else return false;
    }

    public static bool IsUsernameAndDiscriminator(string username)
    {
        if (username != null) return Regex.IsMatch(username, USERNAME_AND_DISCRIMINATOR_PATTERN);
        else return false;
    }

    public static bool IsUsername(string username)
    {
        if (username != null) return Regex.IsMatch(username, USERNAME_PATTERN);
        else return false;
    }
    ...
    public static string Sha256FromSting(string toEncrypt) {
